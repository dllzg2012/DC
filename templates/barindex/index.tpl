   ...
   <div class="AreaL">
	   ...
	   {foreach from=$homeGrp.like item=item key=key}
	     <li style='display:block'>
		  ...
		  <dd >话题：{$item.topicnum}</dd>
		  <dd class="cl_999" >{$item.membernum}人加入</dd>
	     </li>
	     {/foreach}
	   ...
	   <!--/圈子达人-->
		...
		{foreach from=$homeGrp.excelring item=item key=key}
	     <li>
	      <img src="./uploadfiles/user/{$item.photo}" />
		  <dt class="aff6600"><a href="self_zone.php?uid={$item.ID}">{$item.nickname}</a></dt>
		  <dd class="a0693e3">他的热门圈：<a href="grp_single_home.php?ID={$item.gID}">{$item.groupname}</a></dd>
		  <dd>成员：{$item.membernum}</dd>
		  <dd class="cl_999"><a href="grp_single_home.php?ID={$item.gID}">去看看>></a></dd>
	     </li>
	     {/foreach}
