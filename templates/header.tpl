...
<table  border="0" cellspacing="0" cellpadding="0">
  <tr>
    <td>
        <div class="menu4">
            [ <em><A class="menu-hd" href="javascript:void(0)" id="firstcity">{$cities[0].name}</a></em> ]
            <div class="menu-bd">
				<div class="menu-bd-panel">
					<ul id="firstcityarea">
...
<dl>
{foreach from=$cities item=item key=key}
<dd {if $selcityid eq $item.ID}class="sel"{/if}>{$item.name}</dd>
{/foreach}
...
<dd><a href="#">{$item.schoolname}</a></dd>
{/foreach}
...
<p {if $smarty.session.myschooltype neq 4} class="car" {else}class="car backbj"{/if}><a href="flow.php?act=index">购物车</a></p>


<p {if $smarty.session.myschooltype neq 5}class="result" {else}class="result backbj"{/if}><a href="flow.php?act=checkout">去结算</a></p>
</li>                           
</ul>
</div>
