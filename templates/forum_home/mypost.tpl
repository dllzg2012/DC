...
<script src="./templates/scripts/prototype.js" type="text/javascript"></script>
   ...
   {foreach from=$firstforumtype item=item key=key}
    <li><a href="forum_home.php?module=home&type={$item.ID}">{$item.name}</a></li>
   {/foreach}
   </ul>
   {/if}
   </div>
      ...
      <div class="lt_main">
		...
		<!-- 广告 -->
           ...
           <div class="lt_list_rr clearfix">
             ...
             {if $hot}
               <ol class="a666_line" style="float:left">
               {foreach from=$hot item=item key=key} 
               {if $key gt 0 and $key lt 7}
               <li><span class="a999"><a href="zone.php?uid={$item.creater}">{$item.nickname}</a></span>  <a href="forum_home.php?module=replylist&forumid={$item.ID}">{$item.title}</a><br/></li>
              {/if}
              {/foreach}
              </ol>
               {/if}
